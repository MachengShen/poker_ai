...
def make_starting_hand_lossless(starting_hand, short_deck) -> int:
    ...
    for rank in range(2, 15):
        if all(c_rank == rank for c_rank in ranks):
            return 14 - rank

    for rank_1 in range(14, 1, -1):
        for rank_2 in range(rank_1 - 1, 1, -1):
            if rank_1 in ranks and rank_2 in ranks:
                rank = 13 + (11 + rank_1) / 2 * (14 - rank_1) + (rank_1 - rank_2 - 1)
                return rank if suited else 78 + rank
